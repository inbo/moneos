...
ontbrekende_campagnes 

campagnes_NA_or_0 <-
  data_per_datum %>% 
  select(locatiecode, locatie, datum, jaar, maand, aantal_dagen, aantal_fuiken) %>% 
  distinct() %>% 
...
#   filter(
#     # Soort == "bot",
#          # Gebiedcode %in% {uniek_aantal_of_gewicht %>% filter(soort == "brakwatergrondel") %>% pull(locatiecode) %>% unique()},
#          Gebiedcode == "81500000",
#          year(Begindatum) == 2008,
#          month(Begindatum) == 3)

campagnes_nulvangsten <-
  data_per_datum %>% 
