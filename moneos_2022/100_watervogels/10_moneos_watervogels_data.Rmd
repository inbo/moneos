...
gebiedsindeling <- 
  gebiedsgroepenall %>% 
  left_join(gebiedsindeling_T2021, na.rm = TRUE) %>% 
...
Waterbirds_bt_zr_sigma_sigmazs <-
  Waterbirds_bt_zr_sigma %>% 
  bind_rows(waterbirds_sigma_zeeschelde)

#### sigma estuariene gebieden - GGG en ontpolderingen

telzones_sigma_estuarien <- 
  gebiedsgroepenSigma %>%
  filter(gebiedsgroep == "GGG" )
  
      pull(gebiedsnaam) 


telzones_query <-
  paste0("'", telzones_sigma_estuarien, "'", collapse = ",") %>% 
  paste0("(", ., ")")
...
Waterbirds <- Waterbirds_bt_zr_sigma_sigmazs_vallei_valleizs
...
WaterbirdsBT_sigma_estuarien %>% 
  ...
  mutate(niveau3 = case_when(
   ...
   dplyr::select(-c(gebiedsgroep_code, taxongroepcode, wetnaam, teldatum, analyseset)) %>% 
