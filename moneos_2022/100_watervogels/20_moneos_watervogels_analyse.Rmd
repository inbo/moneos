...
check <-data_watervogels %>% 
  ...
  left_join(data_oppzachtslikKRWZS, by = c("gebiedsgroep","jaar")) %>% 
  group_by(telseizoen, jaar, maand, gebiedsgroep, somha) %>% 
  summarise(aantal = sum(aantal, na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(telseizoen, jaar, gebiedsgroep, somha) %>% 
  summarise(aantal = mean(aantal, na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(telseizoen, jaar, gebiedsgroep, somha) %>% 
...
data_watervogels %>% 
  filter(str_detect(gebiedsgroep_code, "ZS4")) %>% 
  filter(maand %in% c(10:12, 1:3)) %>% 
  filter(jaar %in% c(2001,2010,2012:2022)) %>%
  filter(nednaam != "Tafeleend" & nednaam != "Kuifeend") %>%
  filter(trofische_groep != "Npisc" & trofische_groep != "Nherb") %>% 
  left_join(data_oppzachtslikKRWZS, by = c("gebiedsgroep","jaar")) %>% 
...
data_watervogels %>% 
  filter(str_detect(gebiedsgroep_code, "ZS4")) %>% 
  filter(maand %in% c(10:12, 1:3)) %>% 
  filter(jaar %in% c(2001,2010,2012:2021)) %>%
  filter(nednaam != "Tafeleend" & nednaam != "Kuifeend") %>%
  filter(trofische_groep != "Npisc" & trofische_groep != "Nherb") %>% 
  left_join(data_oppzachtslikKRWZS, by = c("gebiedsgroep","jaar")) %>% 
  group_by(jaar, maand, gebiedsgroep, somha, trofische_groep) %>% 
  summarise(aantal = sum(aantal, na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(jaar, trofische_groep,gebiedsgroep, somha) %>% 
  summarise(aantal = mean(aantal, na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(jaar, trofische_groep, gebiedsgroep, somha) %>% 
  summarise(dichtheid = mean(aantal/somha, na.rm = TRUE)) %>% 
  ggplot(aes(ordered(jaar), dichtheid, group = trofische_groep, color = trofische_groep)) + 
  geom_point(aes(shape = gebiedsgroep)) +
...
data_watervogels %>% 
  filter(str_detect(gebiedsgroep_code, "ZS")) %>% 
  filter(jaar %in% c(2001,2010,2013,2016,2019)) %>%
  left_join(data_oppzachtslikKRWZS, by = c("gebiedsgroep","jaar")) %>% 
  filter(!is.na(somha)) %>%
  group_by(jaar, gebiedsgroep, somha) %>% 
