...
# 1.073.368 records wanneer enkel S.samplestatus <> 'REJECTED' (140 extra records)
WV_DB %>% group_by(ProjectCode, Project) %>% summarise(N = n()) 

# specifieke verkenning:
# scheldearm <- filter(WV_DB, 
#                             GebiedsCode == '2050810')
# 
# verken1 <- filter(WV_DB, 
#                          GebiedsCode == "3156629",
#                          Telseizoen == "2025/26")
# 
# verken2 <- filter(WV_DB, 
#                          GebiedsCode == "2091308",
#                          Telseizoen == "2024/25" )
...
Gebieden_ZS_MONEOS <- Gebieden_ZS %>% 
  filter(LocationGroupNaam %in% c("Zeeschelde", "Zijrivieren Zeeschelde")) %>% 
  mutate(rivier = case_when(
...
WV_DB %>% 
  rename_all(tolower) %>% 
  mutate(maand = month(teldatum)) %>%
  filter(gebiedscode %in% Clusters_ZS_MONEOS$gebiedscode) %>% 
  group_by(krwzone,
           rivier,
           telseizoen,
           maand,
...
# 
# datathvGrootschoor %>%
#   filter(!(GebiedsCode == 2091308 & (Telseizoen == "2021/22" | Telseizoen == "2022/23" | Telseizoen == "2023/24"))) %>%
#   filter(!(GebiedsCode == 4143122 & (Telseizoen == "2021/22" | Telseizoen == "2022/23" | Telseizoen == "2023/24"))) %>% 
