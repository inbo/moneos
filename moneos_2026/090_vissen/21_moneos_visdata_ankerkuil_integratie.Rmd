...
data_recent <- 
  ...
  mutate(soort = case_match(soort, 
                            ...
                            "klein zeepaardje" ~ "langsnuitzeepaardje",
...
aantal_NA_of_0_ruw <-
  ...
  select("meting_id", "waarneming_id", "gebiedcode", 
         "locatie", 
         "datum", "jaar", "methode_code", "getij", "zijde", 
         "volume_ue", 
...
data_per_datum <- 
  ...
  group_by(waarneming_id, locatiecode, locatie, datum, jaar, maand, seizoen, getij, soort) %>% 
  summarise(n = n(),
...
campagnes <- 
  data_per_datum %>% 
  select(locatie, jaar, maand, seizoen, getij) %>% 
  distinct()
camp_tot <-
  campagnes %>% 
  expand(locatie, jaar, seizoen, getij)
...
write_xlsx(list(ontbrekende_campagnes = campagnes_ontbrekend),
           path = str_c(pad_data, "controle/campagnes_ontbrekend_ankerkuil.xlsx"))


campagnes_NA_or_0 <-
  data_per_datum %>% 
  group_by(waarneming_id, locatiecode, locatie, datum, jaar, seizoen, getij) %>% 
  summarise(n = sum(n), aantal = sum(aantal, na.rm = TRUE), gewicht = sum(gewicht, na.rm = TRUE)) %>% 
  ungroup() %>% 
  arrange(datum, locatie, getij) %>% 
  filter(if_any(.cols = everything(), .fns = ~is.na(.)))

campagnes_NA_or_0 <-
  campagnes_recent %>% 
  arrange(datum, locatie, getij, zijde) %>% 
  filter(if_any(.cols = c(aantal_uren, volume_ue), \(x) is.na(x) | near(x, 0)))
...
uniek_aantal_of_gewicht <-
  data_per_datum %>% 
  select(locatiecode, locatie, datum, jaar, maand, seizoen, getij, soort, n, aantal, gewicht) %>% 
  arrange(datum, locatie, getij) %>% 
...
write_xlsx(list(#NA_aantal_of_gewicht = uniek_aantal_of_gewicht,
           ...
           path = str_c(pad_data, "controle/controle_ankerkuil_NA_aantal_gewicht_na_imputation.xlsx"))
...
write_xlsx(list(nieuwe_soorten = 
           ...
           path = str_c(pad_data, "nieuwe_soorten_ankerkuil.xlsx"))
...
campagnes_recent <-
  campagnes_recent %>% 
  group_by(waarneming_id, locatie, datum, jaar, getijde = getij, 
         temperatuur, zuurstof,	zuurstof_perc, ph,	turbiditeit, conductiviteit) %>% 
  summarise(uren = sum(aantal_uren),
            volume = sum(volume_ue),
            .groups = "drop") %>% 
  ...
  relocate(seizoen, .before = getijde) %>% 
  arrange(ID_afvissing)

aantallen_recent <-
  data_per_datum %>% 
  select(locatie, datum, getij, soort, aantal) %>% 
  ...
  arrange(datum, locatie) %>% 
  mutate(ID_afvissing = 1:n()) %>% 
  select(ID_afvissing, everything(), -locatie, -datum, -getij) %>% 
  arrange(ID_afvissing)

gewicht_recent <-
  data_per_datum %>% 
  select(locatie, datum, getij, soort, gewicht) %>% 
  pivot_wider(names_from = soort,
              values_from = gewicht,
              names_sort = TRUE,
              values_fill = 0) %>% 
