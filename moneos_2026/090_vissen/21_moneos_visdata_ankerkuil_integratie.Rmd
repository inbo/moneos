...
dilution_NA <-
  data_recent %>% 
  filter(is.na(METI_DILUTION_FACTOR)) %>% 
  select("MetingID", "WaarnemingID", "Gebiedcode", 
         "Gebiednaam", 
         "Begindatum", "Methodenaam", 
         "Soort", 
...
dat_dil <-
  ...
  filter(Gebiedcode %in% dilution_NA$Gebiedcode,
         WaarnemingID %in% dilution_NA$WaarnemingID)
  

data_recent <- 
  data_recent %>% 
  replace_na(list(METI_DILUTION_FACTOR = 1))
...
(soort_NA <-
  gewicht_NA_of_0_ruw %>% 
  filter(round(TAXONAANTAL/METI_DILUTION_FACTOR) == 1) %>% 
  pull(Soort) %>% 
  unique())
...
if (length(soort_NA) > 0) {
  data_lengte <-
    ...
    filter(Soort %in% soort_NA,
           !is.na(TAXONLEN),
           !is.na(TAXONGEW)) %>% 
    select(Soort, TAXONAANTAL, TAXONGEW, TAXONLEN, METI_DILUTION_FACTOR) %>% 
    mutate(TAXONAANTAL = TAXONAANTAL/METI_DILUTION_FACTOR,
           TAXONGEW = TAXONGEW/METI_DILUTION_FACTOR)
  ...
  gewicht_NA <-
    ...
    select(MetingPivotKey, Gebiedcode, Gebiednaam, Begindatum, Month, Year, Methodenaam, 
           TaxonKey, Soort, TAXONAANTAL, TAXONGEW, TAXONTOTGEW, TAXONLEN, METI_DILUTION_FACTOR) %>% 
    ...
    mutate(prop_aantal = TAXONAANTAL/aantal,
           # gewicht_imputed =
           #   case_when(
           #     # round(TAXONAANTAL/METI_DILUTION_FACTOR) > 1 | is.na(TAXONLEN) ~ gewicht*prop_aantal,
           #     TAXONAANTAL > 1 ~ gewicht*prop_aantal,
           #     TRUE ~ exp(int + slope*log(TAXONLEN))
           #   ),
           gewicht_imputed = 
             case_when(
               (!is.na(METI_DILUTION_FACTOR) & round(TAXONAANTAL/METI_DILUTION_FACTOR)) > 1 | is.na(TAXONLEN) ~ gewicht*prop_aantal,
               is.na(METI_DILUTION_FACTOR) & TAXONAANTAL == 1 ~ exp(int + slope*log(TAXONLEN)),
               TRUE ~ exp(int + slope*log(TAXONLEN))*METI_DILUTION_FACTOR
             ))
...
campagnes <- 
  data_per_datum %>% 
  select(locatie, jaar, maand, seizoen, getijde, uren, volume) %>% 
  distinct()
camp_tot <-
  campagnes %>% 
...
write_xlsx(list(#NA_aantal_of_gewicht = uniek_aantal_of_gewicht,
           ...
           path = str_c(pad_data, "controle/controle_NA_aantal_gewicht_na_imputation.xlsx"))
