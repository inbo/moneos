...
pad_figuren <- maak_pad(params$hoofdstuk, "figuren/ankerkuil")
pad_tabellen <- maak_pad(params$hoofdstuk, "tabellen/ankerkuil")
...
data_ankerkuil <- 
  ...
  left_join(soortenlijst %>% 
              select(-inEMSE, -fuiken, -ankerkuil, -opmerking))

```


```{r check eb vloed per locatie en seizoen}

campagne_dupes <-
  data_ankerkuil %>% 
  select(locatie:getijde) %>% 
  distinct() %>% 
  # select(-datum) %>% 
...
get_lab_aantal <-
  function() {
    if (mult_kub > 1)
      lab <- expr(paste("gem. aantal ind. / ", !!mult_kub, m^3))
    else
      lab <- expr(paste("gem. aantal ind. / ", m^3))
    lab
  }
...
get_lab_gewicht <-
  function() {
    if (mult_kub > 1)
      lab <- expr(paste("gem. biomassa [g/", !!mult_kub, m^3, "]"))
    else
      lab <- expr(paste("gem. biomassa [g/", m^3, "]"))
    lab
  }
...
# load(file = str_c("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dUVZJLVE1U2RobWs/INBODATA/PROJECTEN/PRJ_SCHELDE/VNSC/Rapportage_INBO/2023/090_vissen/data/", "soorten_kleur.RData"))
...
my_pal_wes <-
  c(soorten_indeling_kleur$kleur, 
    ...
    "darkslategrey")
names(my_pal_wes) <-
  c(soorten_indeling_kleur$soort, "congeraal", "grondel sp", "haringachtigen", "rest")

colorKey = data.frame(colorName=names(my_pal_wes))
ggplot(data=colorKey, aes(x=1:nrow(colorKey), y = 1, fill=colorName, label=colorName)) +
  geom_tile() +
...
```{r salgroep en dieet grondel sp}

soortenlijst %>% 
  filter(soort %in% c("dikkopje", "brakwatergrondel"))

data_ankerkuil <- 
  data_ankerkuil %>% 
  mutate(salgroep = if_else(soort == "grondel sp", "estuarien residenten", salgroep),
         dieet = if_else(soort == "grondel sp", "benthivoren", dieet))
...
soorten_niet_in_aantal <-
  c("grondel sp", "haringachtigen")

# soorten_niet_in_funcgroep <-
#   c("grondel sp")
soorten_niet_in_funcgroep <-
  c()
...
aantal_soorten_seizoen_locatie <- 
  data_ankerkuil %>% 
  filter(!(soort %in% soorten_niet_in_aantal)) %>% 
  filter(aantal > 0) %>% 
...
ggsave(paste0(pad_figuren, "aantal_soorten_seizoen_locatie.jpg"), width = 10, height = 8,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "aantal_soorten_seizoen_locatie.png"), width = 10, height = 8,
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "aantal_soorten_seizoen.jpg"), width = 10, height = 5,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "aantal_soorten_seizoen.png"), width = 10, height = 5,
       create.dir = TRUE)
...
aantal_soorten_locatie %>% 
  ...
  geom_text(aes(y = soorten_tot + 1, label = soorten_tot),
            position = position_dodge(width = 0.8),
            size = 2, color = "steelblue4", fontface = "bold") +
  labs(y = "aantal soorten",
       fill = "jaar",
       color = "jaar")
...
ggsave(paste0(pad_figuren, "aantal_soorten_locatie.jpg"), width = 12, height = 5,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "aantal_soorten_locatie.png"), width = 12, height = 5,
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "aantal_soorten_totaal.jpg"), width = 6, height = 4,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "aantal_soorten_totaal.png"), width = 6, height = 4,
       create.dir = TRUE)
...
tabel_gevangen_soorten <-
  ...
  pivot_wider(id_cols = soort,
              ...
              values_fill = "")
# %>% 
#   select(soort, all_of(levels_seizoen_locatie))
...
sheets[["nieuwe_soorten"]] <- 
  ...
  select(locatie, OMES, EMSE_niveau3, jaar, seizoen, soort, wetensch_naam, salgroep, dieet, 
         volume, aantal, gewicht, aantal_per_kub, gewicht_per_kub)
...
relatief_aantal_laatste_jaar <- 
  ...
  mutate(relatief_aantal_laatste_jaar = aantal/sum(aantal, na.rm = TRUE),
         relatief_gewicht_laatste_jaar = gewicht/sum(gewicht, na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(soort) %>% 
  mutate(soort2 = ifelse(any(relatief_aantal_laatste_jaar[!is.na(relatief_aantal_laatste_jaar)] >= prop_lim) | any(relatief_gewicht_laatste_jaar[!is.na(relatief_gewicht_laatste_jaar)] >= prop_lim), soort, "rest")) %>% 
  ungroup() %>% 
  ...
  mutate(soort2 = soort2,
         aantal = round(aantal/sum(aantal, na.rm = TRUE)*100)) %>% 
  ungroup() %>% 
  rename(soort = soort2) %>% 
...
#   c(wes_palette("Darjeeling1", 
#                 type = "continuous", 
#                 n = length(levels(relatief_aantal_laatste_jaar$soort)) - 1) %>% 
#       unname(), 
#     "darkslategrey")
# names(my_pal_wes) <-
#   levels(relatief_aantal_laatste_jaar$soort)
...
ggsave(paste0(pad_figuren, "relatief_aantal_gevangen_individuen.jpg"),
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "relatief_aantal_gevangen_individuen.png"),
       create.dir = TRUE)
...
relatief_gewicht_laatste_jaar <- 
  ...
  group_by(seizoen, locatie) %>% 
  mutate(soort2 = soort2,
         gewicht = round(gewicht/sum(gewicht, na.rm = TRUE)*100)) %>% 
  ungroup() %>% 
  rename(soort = soort2) %>% 
  mutate(soort = factor(soort, 
                        levels = unique(soort) %>% 
...
#   c(wes_palette("Darjeeling1", 
#                 type = "continuous", 
#                 n = length(levels(relatief_gewicht_laatste_jaar$soort)) - 1) %>% 
#       unname(), 
#     "darkslategrey")
# names(my_pal_wes) <-
#   levels(relatief_gewicht_laatste_jaar$soort)
...
ggsave(paste0(pad_figuren, "relatieve_biomassa_gevangen_individuen.jpg"),
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "relatieve_biomassa_gevangen_individuen.png"),
       create.dir = TRUE)
...
gemiddeld_diversiteit_EMSE <- 
  ...
  #### in 2025 een net zonder volume in najaar Antwerpen
  filter(!is.na(volume)) |>
  ...
  mutate(relatief_aantal = aantal_per_kub/sum(aantal_per_kub, na.rm = TRUE)) %>% 
  summarise(N = n(),
...
ggsave(paste0(pad_figuren, "diversiteit_S_D_EMSE_marien.jpg"), width = 10, height = 6,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "diversiteit_S_D_EMSE_marien.png"), width = 10, height = 6,
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "diversiteit_S_D_EMSE_diadroom.jpg"), width = 10, height = 6,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "diversiteit_S_D_EMSE_diadroom.png"), width = 10, height = 6,
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "diversiteit_S_D_EMSE_zoet.jpg"), width = 10, height = 6,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "diversiteit_S_D_EMSE_zoet.png"), width = 10, height = 6,
       create.dir = TRUE)
...
relatieve_aantallen_EMSE <- 
  ...
  group_by(jaar, scheldezone, soort, salgroep, dieet, indeling) %>%
  summarise(aantal_per_kub = mean(aantal_per_kub, na.rm = TRUE),
            gewicht_per_kub = mean(gewicht_per_kub, na.rm = TRUE)) %>% 
  ungroup() %>% 
  group_by(jaar, scheldezone, salgroep, dieet, indeling) %>%
  mutate(aantal_per_kub_tot = sum(aantal_per_kub, na.rm = TRUE),
         gewicht_per_kub_tot = sum(gewicht_per_kub, na.rm = TRUE)) %>% 
  ungroup() %>% 
  mutate(relatief_aantal = aantal_per_kub/aantal_per_kub_tot,
         relatief_gewicht = gewicht_per_kub/gewicht_per_kub_tot) %>% 
  ...
  ungroup() %>% 
  group_by(jaar, scheldezone, soort, salgroep, dieet, indeling) %>%
  summarise(aantal_per_kub_tot = mean(aantal_per_kub_tot, na.rm = TRUE),
            gewicht_per_kub_tot = mean(gewicht_per_kub_tot, na.rm = TRUE),
            aantal_per_kub = sum(aantal_per_kub),
            gewicht_per_kub = sum(gewicht_per_kub),
            relatief_aantal = aantal_per_kub/aantal_per_kub_tot,
            relatief_gewicht = gewicht_per_kub/gewicht_per_kub_tot) %>%
  ungroup() %>% 
  add_row(indeling_EMSE_missing %>% 
            expand_grid(jaar = vroegste_jaar:laatste_jaar,
  ...
  mutate(legend = 
           map2(indeling, data, 
                function(ind, dat) {
                  ...
                  plt <-
                    dat %>% 
                    ggplot(aes(jaar, aantal_per_kub*mult_kub, fill = soort)) +
...
ggp_all <-
  relatieve_aantallen_EMSE %>% 
  filter(str_detect(salgroep, "mari")) %>% 
  ggplot(aes(jaar, aantal_per_kub*mult_kub, fill = soort, color = soort)) +
  geom_col(width = 0.7,
           alpha = 0.9,
           size = 0.1) +
  scale_x_continuous(breaks = seq(ifelse(vroegste_jaar %% 2 == 0, vroegste_jaar, vroegste_jaar + 1), laatste_jaar, 2)) +
...
ggsave(paste0(pad_figuren, "relatieve_aantallen_tot_aantal_EMSE_marien.jpg"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "relatieve_aantallen_tot_aantal_EMSE_marien.png"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "relatieve_aantallen_tot_aantal_EMSE_diadroom.jpg"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "relatieve_aantallen_tot_aantal_EMSE_diadroom.png"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "relatieve_aantallen_tot_aantal_EMSE_zoet.jpg"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "relatieve_aantallen_tot_aantal_EMSE_zoet.png"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
  ...
  mutate(legend = 
           map2(indeling, data, 
                function(ind, dat) {
                  ...
                  plt <-
                    dat %>% 
                    ggplot(aes(jaar, gewicht_per_kub*mult_kub, fill = soort)) +
                    geom_col(position = position_stack(),
                             width = 0.7,
                             alpha = 0.9) +
                    scale_x_continuous(breaks = 
...
ggp_all <-
  relatieve_aantallen_EMSE %>% 
  filter(str_detect(salgroep, "mari")) %>% 
  ggplot(aes(jaar, gewicht_per_kub*mult_kub, fill = soort, color = soort)) +
  geom_col(width = 0.7,
           alpha = 0.9,
           size = 0.1) +
  scale_x_continuous(breaks = seq(ifelse(vroegste_jaar %% 2 == 0, vroegste_jaar, vroegste_jaar + 1), laatste_jaar, 2)) +
...
ggsave(paste0(pad_figuren, "relatieve_biomassa_tot_biomassa_EMSE_marien.jpg"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "relatieve_biomassa_tot_biomassa_EMSE_marien.png"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "relatieve_biomassa_tot_biomassa_EMSE_diadroom.jpg"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "relatieve_biomassa_tot_biomassa_EMSE_diadroom.png"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "relatieve_biomassa_tot_biomassa_EMSE_zoet.jpg"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "relatieve_biomassa_tot_biomassa_EMSE_zoet.png"), width = 14, height = 7, bg = "white",
       create.dir = TRUE)
...
sheets_figuren[["aantallen_en_gewichten_EMSE"]] <- 
  ...
  select(jaar, scheldezone, soort, salgroep, dieet, indeling, 
         relatief_aantal, aantal_per_kub, relatief_gewicht, gewicht_per_kub)
...
data_sleutelsoorten_EMSE %>% 
  filter(str_detect(salgroep, "mari")) %>% 
  ggplot(aes(jaar, aantal_per_kub*mult_kub, color = scheldezone, fill = scheldezone)) +
...
ggsave(paste0(pad_figuren, "aantallen_sleutelsoorten_EMSE_marien.jpg"), width = 6, height = 8,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "aantallen_sleutelsoorten_EMSE_marien.png"), width = 6, height = 8,
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "aantallen_sleutelsoorten_EMSE_diadroom.jpg"), width = 7, height = 7,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "aantallen_sleutelsoorten_EMSE_diadroom.png"), width = 7, height = 7,
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "aantallen_sleutelsoorten_EMSE_zoet.jpg"), width = 6, height = 8,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "aantallen_sleutelsoorten_EMSE_zoet.png"), width = 6, height = 8,
       create.dir = TRUE)
...
relatieve_aantallen_salgroep <- 
  ...
  summarise(aantal_per_kub_tot = mean(aantal_per_kub_tot, na.rm = TRUE),
            gewicht_per_kub_tot = mean(gewicht_per_kub_tot, na.rm = TRUE),
            aantal_per_kub = sum(aantal_per_kub, na.rm = TRUE),
            gewicht_per_kub = sum(gewicht_per_kub, na.rm = TRUE),
            relatief_aantal = aantal_per_kub/aantal_per_kub_tot,
            relatief_gewicht = gewicht_per_kub/gewicht_per_kub_tot) %>%
  ungroup() %>% 
  mutate(scheldezone = factor(scheldezone, levels = levels_scheldezone),
         ...
         soort = factor(soort, 
                        levels = unique(soort) %>% 
                          ...
                          setdiff("rest") %>% 
                          c(., "rest")),
         salgroep = factor(salgroep, 
                           levels = c("mariene en estuariene soorten", "diadrome soorten", "zoetwatersoorten")))
...
ggsave(paste0(pad_figuren, "EMSE_alternatief/relatieve_aantallen_tot_aantal_salgroep.jpg"), width = 14, height = 8, bg = "white",
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "EMSE_alternatief/relatieve_aantallen_tot_aantal_salgroep.png"), width = 14, height = 8, bg = "white",
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "EMSE_alternatief/relatieve_biomassa_tot_biomassa_salgroep.jpg"), width = 14, height = 8, bg = "white",
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "EMSE_alternatief/relatieve_biomassa_tot_biomassa_salgroep.png"), width = 14, height = 8, bg = "white",
       create.dir = TRUE)
  ...
  mutate(scheldezone = factor(scheldezone, levels = levels_scheldezone),
         ...
         soort = factor(soort, 
                        levels = unique(soort) %>% 
                          ...
                          c(., "rest")),
         dieet = factor(dieet, 
                           levels = c("benthivoren", "omnivoren", "piscivoren", "planktivoren")))
...
ggsave(paste0(pad_figuren, "EMSE_alternatief/relatieve_aantallen_tot_aantal_dieet.png"), width = 14, height = 8, bg = "white")
...
ggsave(paste0(pad_figuren, "EMSE_alternatief/relatieve_biomassa_tot_biomassa_dieet.jpg"), width = 14, height = 8, bg = "white",
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "EMSE_alternatief/relatieve_biomassa_tot_biomassa_dieet.png"), width = 14, height = 8, bg = "white",
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "EMSE_alternatief/diversiteit_S_D_salgroep.jpg"), width = 10, height = 7,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "EMSE_alternatief/diversiteit_S_D_salgroep.png"), width = 10, height = 7,
       create.dir = TRUE)
...
ggsave(paste0(pad_figuren, "EMSE_alternatief/diversiteit_S_D_dieet.jpg"), width = 7, height = 6,
       create.dir = TRUE)
ggsave(paste0(pad_figuren, "EMSE_alternatief/diversiteit_S_D_dieet.png"), width = 7, height = 6,
       create.dir = TRUE)
...
write_xlsx(sheets,
           paste0(pad_tabellen, "ankerkuil_tabellen_vissen.xlsx"))

write_xlsx(sheets_figuren,
           paste0(pad_tabellen, "ankerkuil_tabellen_figuren.xlsx"))
...
meta_data %>% 
  write_delim(paste0(pad_data, "ankerkuil_meta_data.csv"),
              delim = ";")
