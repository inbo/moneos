...
```{r selectie opmetingen met voldoende punten, eval =FALSE}
...
# data_MONEOS <-
#   data_MONEOS %>%
#   group_by(pick(Salzone:Campagne, MeetCampagneJaar, contains("waterstand_"), contains("afstand_"))) %>% 
#   nest() |> 
#   mutate(heeft_schor = map(data, \(d) any(!is.na(d$ecotoop) & d$ecotoop == "schor"))) |> 
...
figuren_MONEOS <-
  data_MONEOS %>% 
  # filter(year(DATUM) == 2017 & month(DATUM)) |> 
  group_by(REEKSCODE) %>% 
  nest() %>% 
