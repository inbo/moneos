...
```{r data}
Soorten <- read_xlsx(path = str_c(pad_data, "Data_nieuwe_vorm_2026.xlsx"),
                   sheet = "Soorten")
Terr_Sigma_BB <- 
  read_csv(str_c(pad_tabellen, "Terr_Sigma_BB.csv"))
Terr_Sigma_BB_gebiedstype <- 
  read_csv(str_c(pad_tabellen, "Terr_Sigma_BB_gebiedstype.csv"))
Terr_Sigma_AB <- 
...
Terr_Sigma_BB %>% 
  filter(!Soort %in% c("Purperreiger", "Wespendief")) %>% 
  ggplot() +
...
```{r soortgrafiekBB per Typegebied, include = FALSE}
Terr_Sigma_BB_gebiedstype %>%
  filter(Soort == "Wespendief") %>%
...
# ggsave(filename = str_c(pad_figuren, "Scholekster_evolutie_gebiedstype.png"))
...
Terr_Sigma_BB_gebiedstype %>%
  ...
  pull(Soort) %>%
  walk(function(s) {
    p <- Terr_Sigma_BB_gebiedstype %>%
      filter(Soort == s) %>%
      group_by(Soort, TypeGebied, Jaar) %>%
      summarise(Aantal = sum(Aantal), .groups = "drop") %>%
      ggplot(aes(x = Jaar, y = Aantal, color = TypeGebied)) +
      geom_point() +
      geom_line() +
      scale_y_continuous(limits = c(0, NA)) +
      scale_x_continuous(breaks = c(2005, 2010, 2015, 2020)) +
      labs(y = "Aantal territoria", title = s) +
      theme(
        strip.text = element_text(size = 12),
        axis.title = element_text(size = 11),
        axis.title.x = element_blank(),
    ...
    ggsave(
      filename = str_c(pad_figuren, s, "_evolutie_gebiedstype.png"),
      plot = p
    )
    p
  })
...
```{r totale aantallen per soort, include=FALSE}
Terr_Sigma_AB %>% 
  filter(Soort == "Scholekster") %>%
  group_by(Soort, Jaar) %>% 
  summarise(Aantal = sum(Aantal), .groups = "drop")
...
```{r nested data 2025}
data25 <- readxl::read_xlsx(str_c(pad_data, "Data_nieuwe_vorm_2026.xlsx"),
                            sheet = "Data", range = "R1C1:R10000C7") %>% 
  filter(!is.na(Soort), Jaar == 2025)
...
```{r nested data allejaren}
data_all <- readxl::read_xlsx(str_c(pad_data, "Data_nieuwe_vorm_2026.xlsx"),
                            sheet = "Data", range = "R1C1:R10000C7") %>% 
  filter(!is.na(Soort))
# we nest data to make a table for each species-year by using a for loop
# the result is a dataframe with columns Soort(chr), Jaar (num) en data(list). Each list item in data contains a dataframe with Gebied, minimum and maximum.
...
```{r broedlocaties alle jaren, results='asis'}
# The nested data are used in a for loop to produce seperate table in de output html file, notice the use of results='asis' in the header! (without that kable does not work in a for loop)
for(x in unique(data_nested$Soort)){
  a <- data_nested %>% filter(Soort == x)
  for(y in a$Jaar){
...
```{r broedlocaties2 alle jaren, results='asis'}
for(x in unique(data_nested$Soort)){
  a <- data_nested %>% filter(Soort == x) %>% unnest %>% 
    pivot_wider(id_cols = Gebied, names_from = Jaar, values_from = maximum) %>% 
    kable(caption = x, align = "lcc")
