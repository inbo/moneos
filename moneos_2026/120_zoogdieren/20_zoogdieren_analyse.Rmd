...
```{r data}
zoogdier_data_raw <- 
  read_csv(str_c(pad_data, "zoogdier_data_2010_2025.csv"))
```
...
Tot en met rapportage 2021 werden alle waarnemingen opgenomen in de data-analyse. In tabel \@ref(tab:aantal-goedgekeurde-waarnemingen) wordt de dataset beperkt tot goedgekeurde waarnemingen. Voor bever en otter zijn de verschillen niet groot en/of de trends gelijkaardig. Voor bruinvis en zeehonden zijn er zeker na 2016 weinig gevalideerde waarnemingen. We beperken de analysedataset tot gevalideerde data voor bever en otter, voor bruinvis en zeehonden behouden we alle data. **De datalevering bevat alle waarnemingen, ook de niet gevalideerde**.
...
In totaal zijn er `r nb0` gevalideerde beverwaarnemingen in de dataset, waarvan `r nbsp0` geen waarnemingen zijn van dieren, maar wel van sporen of een burcht (`r round(100*nbsp0/nb0, 0)` %). Bij de andere soorten nemen niet-dierwaarnemingen slechts `r round(100*nspnb0/nnb0, 0)` % in.

```{r sporen}
cap <- "Waarnemingen van beversporen of -burcht"
count(bever0_spoor, methode, gedrag) %>% 
  kbl(caption = cap) %>% 
  kable_styling(bootstrap_options = c("condensed", "striped", "hover"))
```

Tabel \@ref(tab:sporen) geeft alle combinaties van methode en gedrag die we bestempelen als niet-dierwaarnemingen. Er zitten enkele exotische combinaties tussen (andere sporen-in kolonie, prenten-zwemmend, sporenbed-foeragerend); het aantal dergelijke waarnemingen is wel erg laag.
...
Onder de goedgekeurde beverwaarnemingen slaan `r round(100*nbsp/nb, 0)` % op sporen (`r nbsp` waarnemingen op een totaal van `r nb`). <br/> <br/>
...
n_seeings <- 
  ...
  pivot_wider(names_from = naam_nl, names_sort = TRUE,
   ...
   arrange(Jaar)
cap <- "Aantal waarnemingen per soort per jaar in de hier geanalyseerde dataset (voor bever en otter enkel gevalideerde waarnemingen, voor de overige soorten alle waarnemingen)"
n_seeings %>% 
  kbl(caption = cap) %>% 
  kable_styling(full_width = FALSE, position = "left",
...
n_indiv |> write_csv(paste0(pad_tabellen, "/aantal_ind.csv"))
...
n_indiv_with_dead %>% 
  write_csv(paste0(pad_tabellen, "/aantal_ind_withdead.csv"))
...
# Aantal otters is VBP-data

Jaren zonder waarneming van otter in wnm.be maar met waarnemingen in VBP: 2015, 2016, 2017, 2018
Jaren zonder waarneming van otter in VBP maar met waarnemingen in wnm.be: 2014
n waarneming VBP > n waarnemingen wnm.be: 2015 (1), 2019 (14)
n waarneming wnm.be > n waarnemingen VBP: 2020 (7), 2021 (60), 2022 (64), 2023 (96), 2024 (77) en 2025 (55)
...
n_seeings_otter_VBP <- 
  ...
  summarise(.by = c(naam_nl, Year), 
            # n_dieren = sum(`Individual count`),
            n_waarnemingen = n()) |> 
  full_join(tibble(Jaar = c(2010:2025)),
            by = c("Year" = "Jaar")) |> 
  replace_na(list(
    n_waarnemingen = 0,
    naam_nl = "otter")) |> 
...
cap <- "Aantal waarnemingen van otter per jaar in de VBP-dataset "
n_seeings_otter_VBP %>% 
  kbl(caption = cap) %>% 
  kable_styling(full_width = FALSE, position = "left",
                bootstrap_options = c("condensed", "striped"))  
...
# make list of tibbles for each group in zoogd_grouped; each tibble gives the amount of seeings for a certain year-species
utm_n_list <-
  zoogd_grouped%>% 
...
otter_VBP_grouped <- 
  ...
  rename(Jaar = Year) |> 
  summarise(aantal = n(), 
            radius = sum(aantal)^0.5,
            .by = c(naam_nl, Jaar, TAG)) |> 
  group_by(Jaar)
...
utm_n_list_VBP <-
  ...
  group_split(.keep = TRUE)
groupkeys <-  # get names of eacht tibble ("naam-nl_jaar")
  group_keys(otter_VBP_grouped)
names(utm_n_list_VBP) <- groupkeys$Jaar # names tiblles in list

# make list of simple features
utm_n_list2_VBP <- map(utm_n_list_VBP, merge, utm1_c) # merge to utm centroid
utm_n_list_sf_VBP <- map(utm_n_list2_VBP, st_as_sf, sf_column_name = "geometry") # make sf


utm_tmmap_list_VBP <- map(utm_n_list_sf_VBP, mapf) # make tmap objects
utm_tmmap_list_VBP[[1]]
# save all maps 
for(x in names(utm_tmmap_list_VBP)){
  a <- utm_tmmap_list_VBP[[x]]
  tmap_save(a, filename = str_c(pad_figuren, "VBP_otter", x, ".png"))
}
```
...
```{r mapbever2024, fig.cap="Aantal waarnemingen van Europese bever per UTM-km-hok in 2024."}
knitr::include_graphics(paste0(pad_figuren, "Europese bever_2024.png"))
```

```{r mapbever2015, fig.cap="Aantal waarnemingen van Europese bever per UTM-km-hok in 2025."}
...
bever_evol <- 
  ...
  mutate(EersteJaarf = factor(EersteJaar, levels = c(2010:2025)),
         EersteJaarCat = 
  ...
  arrange((EersteJaar))
pal <- cols4all::c4a("-brewer.yl_or_br", n = 7)
(kolononisatie_fig <- 
  map_basis +
  tm_shape(bever_evol) + 
...
pal <- cols4all::c4a("-brewer.yl_or_br", n = 16)
(map_basis +
  bever_evol %>% 
  filter(!is.na(EersteJaar)) %>% 
  tm_shape() + 
  tm_fill(fill = "EersteJaarf", 
          fill.scale = 
            tm_scale_categorical(values = pal, labels = c(2010:2025)),
...
```{r mapotter2024, fig.cap="Aantal waarnemingen van Europese otter per UTM-km-hok in 2024."}
knitr::include_graphics(paste0(pad_figuren, "Europese otter_2024.png"))
```

```{r mapotter2025, fig.cap="Aantal waarnemingen van Europese otter per UTM-km-hok in 2025."}
...
GewoneZeehond2025 <- 
  ...
  arrange(Datum) 
GewoneZeehond2025 %>% 
  kbl(align = "lcllll", caption = "Tabel XX: overzicht van alle waarnemingen van gewone zeehond in 2025 in de Zeeschelde en haar bijrivieren.") %>% 
...
GewoneZeehond2025 %>%
  write_csv(paste0(pad_tabellen, "/zeehond2025.csv"))
```
...
GrijzeZeehond2025 <- 
  ...
  arrange(Datum) 
GrijzeZeehond2025 %>% 
  kbl(align = "lcllll", caption = "Tabel XX: overzicht van alle waarnemingen van grijze zeehond in 2025 in de Zeeschelde en haar bijrivieren.") %>% 
  kable_styling(full_width = TRUE, bootstrap_options = c("condensed")) %>% 
  column_spec(1, width = "2.5cm") %>% 
  column_spec(2, width = "1.5cm") %>% 
  column_spec(3, width = "2.5cm")
...
GrijzeZeehond2025 %>%
  write_csv(paste0(pad_tabellen, "/grijzezeehond2025.csv"))
```
...
Bruinvis2025 <- 
  ...
  arrange(Datum) 
Bruinvis2025 %>% 
  kbl(align = "lcllll", caption = "Tabel XX: overzicht van alle waarnemingen van bruinvis in 2025 in de Zeeschelde en haar bijrivieren.") %>% 
  kable_styling(full_width = TRUE, bootstrap_options = c("condensed")) %>% 
  column_spec(1, width = "2.5cm") %>% 
  column_spec(2, width = "1.5cm") %>% 
  column_spec(3, width = "2.5cm")
...
Bruinvis2025 %>%
  write_csv(paste0(pad_tabellen, "/bruinvis2025.csv"))
```
