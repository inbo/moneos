...
```{r relatie-aantal-gewicht, fig.height=16, fig.width=10}
...
```{r gemiddeld-per-seizoenen-en-locatie, fig.height=8, fig.width=6}

data_ankerkuil %>% 
  ggplot(aes(ordered(jaar), aantal_per_kub, fill = seizoen)) +
  geom_col(alpha = 0.8,
           width = 0.5) +
  geom_text(aes(y = aantal_per_kub + 0.3, label = round(aantal_per_kub, 2)), 
            size = 4) +
