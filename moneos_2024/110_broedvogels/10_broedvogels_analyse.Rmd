...
```{r data}
Soort <- read_csv(str_c(pad_data, "Soort.csv"))
Terr_Sigma_BB <- 
...
data23 <- readxl::read_xlsx(str_c(pad_data, "Data_nieuwe_vorm_2023.xlsx"), 
                            range = "A1:G350") %>% 
  filter(!is.na(Soort))
...
data_nested <- 
  data23  %>% 
  arrange(Soort) %>% 
  nest(data = c(Gebiedsnaam, minimum, maximum), .by = c(Soort, Jaar))
# data_nested$data[1]
...
```{r broedlocaties 2021, results='asis'}
# The nested data ars used in a for loop to produce seperate table in de output html file, notice the use of results='asis' in the header! (witjout that kable does not work in a for loop)
for(x in data_nested$Soort){
  a <- data_nested %>% filter(Soort == x)
  for(y in a$Jaar){
    cap <- str_c("Overzicht van broedlocaties en aantal broedkoppels van ",
               x, " in ", y)
    b <- data_nested$data[data_nested$Soort == x & data_nested$Jaar == y] %>% 
