---
params:
  hoofdstuk: "130_sedimentatie_erosie"
knit: (function(inputFile, encoding) {
        rmarkdown::render(inputFile,
                          encoding=encoding,
                          output_dir = paste0("../", 
                          rmarkdown::yaml_front_matter(inputFile)$params$hoofdstuk, 
                          "/output")
                          )})
title: "MONEOS hoofdstuk sedimentatie - erosie"
output:
  bookdown::pdf_book:
    base_format: INBOmd::inbo_rapport
    style: INBO
    tocdepth: 2
    lof: TRUE
    lot: TRUE
    floatbarrier: subsection
bibliography: references.bib
link-citations: TRUE

header-includes: 
  \usepackage{float}
  \usepackage{caption}
---


```{r 130-hoofdstuk, include=FALSE}

hoofdstuk <- "130_sedimentatie_erosie"

```


```{r 130-setup, include=FALSE}

knitr::opts_chunk$set(echo = FALSE, error=FALSE, warning=FALSE, message=FALSE, cache=FALSE, fig.pos = "H", out.extra = '')
knitr::opts_knit$set(eval.after = "fig.cap")

```


```{r 130-libraries}

library(tidyverse)
library(readxl)
library(kableExtra)
library(INBOtheme)

```


```{r 130-pad}

# inlezen van variabelen
# pad naar data : pad_data
# pad naar tabellen : pad_tabellen
# pad naar figuren : pad_figuren

source("../pad.R")

pad_data <- maak_pad(hoofdstuk, "data")
pad_figuren <- maak_pad(hoofdstuk, "figuren")
pad_tabellen <- maak_pad(hoofdstuk, "tabellen")

```


```{r 130-meta-data}

# aanpassen indien er meta-data zijn

# meta_data <- 
#   read_delim(paste0(pad_data, "meta_data.csv"),
#              delim = ";")
#   
# for(i in 1:nrow(meta_data)){
#   ##first extract the object value
#   tempobj=meta_data$waarde[i]
#   ##now create a new variable with the original name of the list item
#   eval(parse(text=paste(meta_data$naam[i],"= tempobj")))
# }

```



# Sedimentatie en erosie op punten en raaien

Fiche nummer: S-MD-V002 Topo-bathymetrie - Sedimentatie en erosie op punten en raaien

__Alexander Van Braeckel, Ruben Elsen, Dimitri Buerms & Joram De Beukelaer__


## Inleiding

Figuur \@ref(fig:130-figuur-overzicht-raaien).

```{r 130-figuur-overzicht-raaien, fig.cap=caption_overzicht, out.width="100%"}

caption_overzicht <- "Situering van de 24 vaste MONEOS-raaien binnen de Zeeschelde. Zie Tabel **XXX** voor naamgeving."

knitr::include_graphics(paste0(pad_figuren, "0_OverzichtskaartMoneosraaien_Zeeschelde.jpg"))

```

<br/>


## Methode

Tabel \@ref(tab:130-tabel-overzicht-raaien) in \@ref(BL1) Bijlage.


## Exploratieve data-analyse


```{r 130-figuur-GBSa, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai GBSa"

knitr::include_graphics(paste0(pad_figuren, "1_MONEOSDEELGBSa.png"))

```

<br/>



```{r 130-figuur-ODa, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai ODa"

knitr::include_graphics(paste0(pad_figuren, "2_MONEOSDEELODa.png"))

```

<br/>


```{r 130-figuur-ODaslik, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai ODaslik"

knitr::include_graphics(paste0(pad_figuren, "3_MONEOSDEELODaslik.png"))

```

<br/>


```{r 130-figuur-DO, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai DO"

knitr::include_graphics(paste0(pad_figuren, "4_MONEOSDEELDO.png"))

```

<br/>


```{r 130-figuur-GSb, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai GSb"

knitr::include_graphics(paste0(pad_figuren, "5_MONEOSDEELGSb.png"))

```

<br/>


```{r 130-figuur-GSc, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai GSc"

knitr::include_graphics(paste0(pad_figuren, "6_MONEOSDEELGSc.png"))

```

<br/>


```{r 130-figuur-LH, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai LH"

knitr::include_graphics(paste0(pad_figuren, "7_MONEOSDEELLH.png"))

```

<br/>


```{r 130-figuur-LP, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai LP"

knitr::include_graphics(paste0(pad_figuren, "8_MONEOSDEELLP.png"))

```

<br/>


```{r 130-figuur-KPb, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai KPb"

knitr::include_graphics(paste0(pad_figuren, "9_MONEOSDEELKPb.png"))

```

<br/>


```{r 130-figuur-KPe, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai KPe"

knitr::include_graphics(paste0(pad_figuren, "10_MONEOSDEELKPe.png"))

```

<br/>


```{r 130-figuur-GW, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai GW"

knitr::include_graphics(paste0(pad_figuren, "11_MONEOSDEELGW.png"))

```

<br/>


```{r 130-figuur-HO, fig.cap=caption_raai, out.width="100%"}

caption_raai <- "Raai HO"

knitr::include_graphics(paste0(pad_figuren, "12_MONEOSDEELHO.png"))

```

<br/>


## Bijlage {#BL1}

<!-- Tabel \@ref(tab:130-tabel-overzicht-raaien). Overzicht van de MONEOS raaien langs de Zeeschelde. Saliniteitszone, campagne, raaicode, naam en X,Y coördinaat (Lambert 72). -->
Tabel (\#tab:130-tabel-overzicht-raaien). Overzicht van de MONEOS raaien langs de Zeeschelde. Saliniteitszone, campagne, raaicode, naam en X,Y coördinaat (Lambert 72).

|  <!-- -->   | 
| :------------ | 
| ![image](`r paste0(pad_tabellen, "Tabel_MONEOSraaien.jpg")`){out.width="100%"}    |


<!-- ```{r 130-tabel-caption-raaien, out.width="100%"} -->

<!-- caption_tabel_raaien <- -->
<!--   "Overzicht van de MONEOS raaien langs de Zeeschelde. Saliniteitszone, campagne, raaicode, naam en X,Y coördinaat (Lambert 72)." -->

<!-- data_frame(![image](paste0(pad_tabellen, "Tabel_MONEOSraaien.jpg"))) %>% -->
<!-- # NULL %>% -->
<!--   knitr::kable(caption = caption_tabel_raaien) -->

<!-- knitr::include_graphics(paste0(pad_tabellen, "Tabel_MONEOSraaien.jpg")) -->

<!-- ``` -->


<!-- ```{r 130-tabel-overzicht-raaien, out.width="100%"} -->

<!-- knitr::include_graphics(paste0(pad_tabellen, "Tabel_MONEOSraaien.jpg")) -->

<!-- ``` -->

<br/>


## Refenties

