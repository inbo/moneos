...
```{r correcties soortnamen recente data}
...
if (length(soort_NA) > 0) {
  ...
  gewicht_NA <-
    ...
    mutate(prop_aantal = TAXONAANTAL/aantal,
           gewicht_imputed = 
             case_when(
               TAXONAANTAL == 1 & !is.na(TAXONLEN) ~ exp(int + slope*log(TAXONLEN)),
               TRUE ~ gewicht*prop_aantal))
...
write_xlsx(list(nieuwe_soorten = 
           ...
           path = str_c(pad_data, "nieuwe_soorten_fuiken.xlsx"))
