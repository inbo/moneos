...
data2024sed <- read_excel(paste0(pad_data, "Sediment2024.xlsx"))

#rapportage spatial data alleen
#datastructuur aanpassen aan sedimentrapportage-file

table(data2024sed$CampagneCode)
table(data2024sed$Fysiotoop)

data2024 <- data2024sed %>% 
  dplyr::filter (CampagneCode == "Spatial2024") %>% 
  dplyr::mutate(
    campagne = "SP24",
    jaar = 2024,
    tidaal = case_when(
      ...
      TRUE ~ "intertidaal"),
    waterloop = case_when(
      str_detect(Omes, "Durme") ~ "Durme",
      str_detect(Omes, "Rupel") ~ "Rupel",
      str_detect(Omes, "19 trGM") ~"Zeeschelde Ia",
      str_detect(Omes, "TijarmZw") ~"Zeeschelde Ib",
      TRUE ~ KRWzone
        ),
    waterloop = str_replace(waterloop, "Zeeschelde III \\+ Rupel", "Zeeschelde III"),
    slib = `0` + `2` + `4` + `16`
    )  %>% 
  rename(locatie = LocatieCode,
         X = POINT_X,
...
table(data2024$waterloop)
table(data2024$SalZone)
table(data2024$Omes)
...
##data toevoegen aan dataset vorige rapportage
  data <- read_excel(paste0(pad_data, "S_DS_V_004a_sediment_data2008_2023_rapportage2024.xlsx"))
