...
# - ik heb ik een extra join toegevoegd op een subquery "loc" waarmee de 
...
#   dat dus hier in de query zelf doen 
...
verken <- dplyr::filter(WV_DB, GebiedsCode == "3151403" & 
                          Telseizoen == "2010/11" &
                          Telling == "Oktober")
...
listspecies <- WV_DB %>%
  ...
  distinct(NedNaam)
##all rare species, not in high numbers present (of no influence for trophic trends)
...
Gebieden_ZS <- Gebieden_ZS %>% 
  left_join(Clusters_ZS_MONEOS) #suffix suffix = c("", "") betekent dat alle 'dubbele' kolommen worden weggelaten
