...
pad_figuren <- str_c(pad_figuren, "fuiken/")
...
-   *G:/Mijn Drive/INBODATA/PROJECTEN/PRJ_SCHELDE/VNSC/Rapportage_INBO/2022/090_vissen/data/fuiken_VLIZ_2012_2021.csv*
...
fuiken_aantal <- 
  ...
  left_join(read_xlsx(str_c(pad_data, filename),
                      sheet = "aantallen")) %>% 
  select(-ID_afvissing) %>% 
  select(locatie, OMES, EMSE_niveau3, datum, jaar, maand, everything())
...
get_lab_aantal <-
  function() {
    if (mult_fuik > 1)
      ...
      lab <- expr(paste("gem. aantal ind. / fuikdagen"))
    lab
  }
...
gemiddeld_aantal_EMSE %>% 
  ...
  mutate(aantal_per_fuikdag = aantal_per_fuikdag*mult_fuik) %>%
  # ggplot(aes(jaar, aantal_per_fuikdag, fill = ordered(jaar), color = ordered(jaar))) +
  ggplot(aes(jaar, aantal_per_fuikdag)) +
...
# ggsave(paste0(pad_figuren, "aantallen_EMSE_marien.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "aantallen_EMSE_diadroom.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "aantallen_EMSE_zoet.jpg"), width = 10, height = 6)
...
gemiddeld_gewicht_EMSE <- 
  ...
  filter(in_indeling) %>% 
  group_by(jaar, scheldezone, seizoen, locatie, salgroep, dieet, indeling) %>%
  summarise(gewicht_per_fuikdag = sum(gewicht_per_fuikdag)) %>% 
  ungroup() %>% 
  group_by(jaar, scheldezone, salgroep, dieet, indeling) %>%
  summarise(gewicht_per_fuikdag = mean(gewicht_per_fuikdag)) %>% 
  ungroup() %>% 
  add_row(tibble(gewicht_per_fuikdag = NA, 
                 salgroep = "diadrome soorten", 
                 indeling = "Diadr -- Benth", 
                 jaar = vroegste_jaar:laatste_jaar) %>% 
            expand_grid(scheldezone = c("SG", "OH", "ZL"))) %>% 
...
gemiddeld_gewicht_EMSE %>% 
  filter(str_detect(salgroep, "mari")) %>% 
  mutate(gewicht_per_fuikdag = gewicht_per_fuikdag*mult_fuik) %>%
  # ggplot(aes(jaar, gewicht_per_fuikdag, fill = ordered(jaar), color = ordered(jaar))) +
  ggplot(aes(jaar, gewicht_per_fuikdag)) +
  geom_col(position = position_dodge(),
           color = "steelblue4",
           size = 0.5,
           width = 0.7,
...
# ggsave(paste0(pad_figuren, "biomassa_EMSE_marien.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "biomassa_EMSE_diadroom.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "biomassa_EMSE_zoet.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "diversiteit_N_EMSE_marien.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "diversiteit_N_EMSE_diadroom.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "diversiteit_N_EMSE_zoet.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "diversiteit_H_shannon_EMSE_marien.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "diversiteit_H_shannon_EMSE_diadroom.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "diversiteit_H_shannon_EMSE_zoet.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "diversiteit_D_shannon_EMSE_marien.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "diversiteit_D_shannon_EMSE_diadroom.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "diversiteit_D_shannon_EMSE_zoet.jpg"), width = 10, height = 6)
...
# ggsave(paste0(pad_figuren, "relatieve_aantallen_EMSE_marien.jpg"), width = 14, height = 7, bg = "white")
...
# ggsave(paste0(pad_figuren, "relatieve_aantallen_EMSE_diadroom.jpg"), width = 14, height = 7, bg = "white")
...
# ggsave(paste0(pad_figuren, "relatieve_aantallen_EMSE_zoet.jpg"), width = 14, height = 7, bg = "white")
  ...
  mutate(indeling = factor(indeling, levels = levels_indeling),
         ...
         soort = factor(soort, 
                        levels = unique(soort) %>% 
                          ...
                          setdiff("rest") %>% 
                          c(., "rest"))) %>% 
  select(-aantal_per_fuikdag, -gewicht_per_fuikdag) %>% 
  ...
  mutate(legend = 
           map2(indeling, data, 
                function(ind, dat) {
                  ...
                  plt <-
                    dat %>% 
                    ggplot(aes(jaar, relatief_aantal*aantal_per_fuikdag*mult_fuik, fill = soort)) +
                    geom_col(position = position_stack(),
                             width = 0.7,
                             alpha = 0.9) +
                    scale_x_continuous(breaks = 
...
ggp_all <-
  relatieve_aantallen_EMSE %>% 
  filter(str_detect(salgroep, "mari")) %>% 
  ggplot(aes(jaar, relatief_aantal*aantal_per_fuikdag*mult_fuik, fill = soort, color = soort)) +
  geom_col(width = 0.7,
           alpha = 0.9,
           size = 0.1) +
  scale_x_continuous(breaks = seq(ifelse(vroegste_jaar %% 2 == 0, vroegste_jaar, vroegste_jaar + 1), laatste_jaar, 2)) +
  ...
  mutate(legend = 
           map2(indeling, data, 
                function(ind, dat) {
                  ...
                  plt <-
                    dat %>% 
                    ggplot(aes(jaar, relatief_gewicht*gewicht_per_fuikdag*mult_fuik, fill = soort)) +
                    geom_col(position = position_stack(),
                             width = 0.7,
                             alpha = 0.9) +
                    scale_x_continuous(breaks = 
...
ggp_all <-
  relatieve_aantallen_EMSE %>% 
  filter(str_detect(salgroep, "mari")) %>% 
  ggplot(aes(jaar, relatief_gewicht*gewicht_per_fuikdag*mult_fuik, fill = soort, color = soort)) +
  geom_col(width = 0.7,
           alpha = 0.9,
           size = 0.1) +
  scale_x_continuous(breaks = seq(ifelse(vroegste_jaar %% 2 == 0, vroegste_jaar, vroegste_jaar + 1), laatste_jaar, 2)) +
...
#  filter(map2_lgl(scheldezone_rap, scheldezone, ~str_detect(.x, .y))) %>% 
  ...
  summarise(aantal_per_fuikdag = mean(aantal_per_fuikdag)*mult_fuik) %>% 
  ungroup() %>% 
  mutate(scheldezone = factor(scheldezone, levels = levels_scheldezone),
         scheldezone = recode_factor(scheldezone, !!!levels_scheldezone_full)) %>% 
  ggplot(aes(jaar, aantal_per_fuikdag, color = scheldezone, fill = scheldezone)) +
...
```{r sleutelsoorten biomassa, eval=FALSE}
...
#  filter(map2_lgl(scheldezone_rap, scheldezone, ~str_detect(.x, .y))) %>% 
  group_by(jaar, soort, scheldezone) %>% 
  summarise(gewicht_per_fuikdag = mean(gewicht_per_fuikdag)*mult_fuik) %>% 
  ungroup() %>% 
  mutate(scheldezone = factor(scheldezone, levels = levels_scheldezone),
         scheldezone = recode_factor(scheldezone, !!!levels_scheldezone_full)) %>% 
  ggplot(aes(jaar, gewicht_per_fuikdag, color = scheldezone, fill = scheldezone)) +
  geom_col(position = position_dodge(width = 0.9),
           linewidth = 0.5,
           width = 0.7,
           alpha = 0.8) +
...
ggsave(paste0(pad_figuren, "alternatief/relatieve_aantallen_tot_aantal_salgroep.jpg"), width = 14, height = 8, bg = "white")
...
ggsave(paste0(pad_figuren, "alternatief/relatieve_biomassa_tot_biomassa_salgroep.jpg"), width = 14, height = 8, bg = "white")
...
ggsave(paste0(pad_figuren, "alternatief/relatieve_aantallen_tot_aantal_dieet.jpg"), width = 14, height = 8, bg = "white")
...
ggsave(paste0(pad_figuren, "alternatief/relatieve_biomassa_tot_biomassa_dieet.jpg"), width = 14, height = 8, bg = "white")
...
ggsave(paste0(pad_figuren, "alternatief/diversiteit_S_D_salgroep.jpg"), width = 10, height = 7)
...
ggsave(paste0(pad_figuren, "alternatief/diversiteit_S_D_dieet.jpg"), width = 7, height = 6)
