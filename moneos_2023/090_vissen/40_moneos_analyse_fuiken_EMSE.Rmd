...
locatie_scheldezone <- 
  ...
  mutate(scheldezone = case_when(
    scheldezone_full == "saliniteitsgradient" ~ "SG",
    scheldezone_full == "oligohalien" ~ "OH",
    scheldezone_full == "zoet lang verblijf" ~ "ZL",
    scheldezone_full == "zoet kort verblijf" ~ "ZK"))
...
levels_scheldezone_full <-
  c(SG = "saliniteitsgradient", OH = "oligohalien", ZL = "zoet lang verblijf", ZK = "zoet kort verblijf")
                ...
                function(ind, dat) {
                  ...
                  plt <-
                    ...
                    scale_x_continuous(breaks = 
                                         seq(ifelse(vroegste_jaar %% 2 == 0, vroegste_jaar, vroegste_jaar + 1), laatste_jaar, 2)) +
                    scale_fill_manual(values = pal_split) +
                    labs(y = expr(paste("gem. aantal ind. / ", !!mult_fuik, "fuikdagen")),
                         fill = ind) + 
                    guides(fill=guide_legend(nrow=4)) +
                    theme(legend.justification = c(0,1),
                          legend.box.background = element_rect(fill = 'transparent', colour = NA_character_))
                ...
                function(ind, dat) {
                  ...
                  plt <-
                    ...
                    scale_x_continuous(breaks = 
                                         seq(ifelse(vroegste_jaar %% 2 == 0, vroegste_jaar, vroegste_jaar + 1), laatste_jaar, 2)) +
                    scale_fill_manual(values = pal_split) +
                    labs(y = expr(paste("gem. biomassa [g/", !!mult_fuik, "fuikdagen", "]")),
                         fill = ind) + 
                    guides(fill=guide_legend(nrow=4)) +
                    theme(legend.justification = c(0,1),
                          legend.box.background = element_rect(fill = 'transparent', colour = NA_character_))
...
ggp_all <-
  ...
  scale_color_manual(values = my_pal_wes) +
  labs(# title = "estuariene soorten en mariene migranten",
       y = expr(paste("gem. biomassa [g/", !!mult_fuik, "fuikdagen", "]"))) + 
  facet_grid(indeling ~ scheldezone, scales = "free_y") +
  theme(legend.position = "none",
        axis.text = element_text(size = 8))
...
#  filter(map2_lgl(scheldezone_rap, scheldezone, ~str_detect(.x, .y))) %>% 
  ...
  scale_x_continuous(breaks = seq(ifelse(vroegste_jaar%%2 == 0, vroegste_jaar, vroegste_jaar+1), laatste_jaar, 2)) +
  labs(# title = "estuariene soorten en mariene migranten",
       y = expr(paste("gem. aantal ind. / ", !!mult_fuik, "fuikdagen"))) + 
  facet_wrap(~soort,
             scales = "free_y",
             ncol = 2) +
