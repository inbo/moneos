...
```{r controle individu en TAXONTOTGEW in ruwe data}

individu_TAXONTOTGEW <-
  data_recent %>% 
...
data_per_datum <- 
  ...
  summarise(n = n(),
            gewicht = sum(if_else(is.na(gewicht_ind), 
                          ...
                          na.rm = FALSE),
            # gewicht = sum(if_else(is.na(gewicht_ind), 
            #                       gewicht_tot, gewicht_ind*aantal),
            #               na.rm = FALSE),
...
campagnes <- 
  data_per_datum %>% 
  select(locatie, jaar, maand, getijde) %>% 
  distinct() %>% 
  mutate(seizoen = 
           case_when(
           maand %in% 3:5 ~ "voorjaar",
...
uniek_aantal_of_gewicht <-
  data_per_datum %>% 
  select(locatiecode, locatie, datum, jaar, maand, getijde, uren, volume, soort, n, aantal, gewicht) %>% 
  arrange(datum, locatie, getijde) %>% 
