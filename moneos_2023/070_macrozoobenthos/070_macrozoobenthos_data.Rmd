...
zero_densiteit_nonzero_biomassa <-
  ...
  filter(densiteit == 0,
         biomassa != 0) %>% 
  select(locatie, KRWzone, fysiotoop, soort, densiteit, biomassa)
...
data_macrobenthos_recent1 <- data_macrobenthos_recent %>% 
  ...
  left_join(taxalijst, by = "soort") %>% 
    dplyr::select(jaar, fysiotoop, locatie=staal, soort, densiteit = N_per_m2, biomassa = Biomassa_per_m2, Taxa_groep, KRWzone)
