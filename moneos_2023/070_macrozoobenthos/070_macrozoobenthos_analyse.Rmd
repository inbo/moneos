...
```{r Wormendeterminatiejaar - alleen uit te voeren als er OID zijn verwerkt}
...
```{r bijdrage taxa groepen Zeeschelde}
...
```{r bijdrage van bivalv soorten aan Bivalvia doorheen de tijd}
...
```{r VOOR OID jaren, OID verwerking}
...
```{r Species accumulation OID: vgl tussen de 5 jaren per ZS segment}
...
```{r Species accumulation OID: vgl tussen de 5 jaren per zijrivier}
#for loop om df te maken bruikbaar voor vegan
var_list = unique(OIDsp$jaar)
...
```{r Species accumulation OID: vgl tussen de 5 jaren hele ZS}
#for loop om df te maken bruikbaar voor vegan
var_list1 <- OIDsp %>% 
  dplyr::ungroup() %>% 
  dplyr::filter(systeem == "Zeeschelde") %>% 
