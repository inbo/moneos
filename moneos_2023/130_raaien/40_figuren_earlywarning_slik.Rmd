...
data_EW_interp_full <-
  data_EW_sel %>%
  group_by(REEKSCODE) %>% 
  mutate(steps = range_dist/aantal_dist,
         steps2 = mean(range_year/aantal_year),
         steps3 = min(range_year/aantal_year),
         distance_prof = round(distance_prof/steps2)*steps2) %>% 
  ungroup()
...
data_EW_interp <-
  data_EW_interp_full %>% 
  group_by(REEKSCODE, afstand_grens, Salzone, Omes, Year, MeetCampagneJaar, distance_prof, waterstand_DD_25, waterstand_DD_35) %>% 
  summarise(HOOGTE__MT = mean(HOOGTE__MT, na.rm = TRUE)) %>% 
  ungroup()
