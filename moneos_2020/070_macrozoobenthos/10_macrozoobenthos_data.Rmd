...
data_macrobenthos_recent <- 
  read_excel(paste0(pad_data, "macrobenthos_spatial_2018.xlsx")) %>% 
  mutate(jaar = year(datum)) %>% 
  rename(fysiotoop = ecotoop_werkelijk,
         tidaal = tidaal_werkelijk)
...
data_macrobenthos_recent <-
  data_macrobenthos_recent %>% 
  mutate(fysiotoop = if_else(is.na(fysiotoop), ecotoop_gepland, fysiotoop),
         fysiotoop = if_else(staal == "GM18_03", ecotoop_gepland, fysiotoop),
         tidaal = if_else(str_detect(fysiotoop, "subtidaal"), "sub", "inter"))
